...
export const GreenShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    const updateColor = () => {
      ...
      setBackgroundColor(isDark ? [0.05, 0.05, 0.05] : [0.98, 0.97, 0.94]);
    };
    updateColor();
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        // Green/teal gradient - matching site colors
        vec3 foregroundColor=vec3(0.18 + v.x * 0.2, 0.42 + v.y * 0.3, 0.31 + v.x * v.y * 0.2);
        vec3 color=mix(uBackgroundColor,foregroundColor,mask);
        color=mix(color,vec3(0.9),paintCircle(uv,center,radius,.003).r);
