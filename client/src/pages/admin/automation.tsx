...
export default function AutomationPage() {
  ...
  const allOff = Object.values(states).every(v => !v);
      ...
      <div className="space-y-6">
        ...
        <div className={cn(
          ...
          {allOff ? (
            ...
            <><CheckCircle2 className="h-4 w-4 shrink-0" /><span>All {SERVICES.length} automations are <strong>active</strong> and running on schedule.</span></>
          ) : (
            <><CheckCircle2 className="h-4 w-4 shrink-0 text-amber-500" /><span className="text-amber-600 dark:text-amber-400"><strong>{Object.values(states).filter(Boolean).length}</strong> of {SERVICES.length} automations are currently enabled.</span></>
          )}
        </div>

        {/* Cost warning if any high-cost are on */}
        {SERVICES.filter(s => s.costLevel === "high" && states[s.id]).length > 0 && (
          <div className="flex items-start gap-3 px-4 py-3 rounded-lg border border-amber-500/20 bg-amber-500/5 text-sm text-amber-600 dark:text-amber-400">
            ...
            <span>
              ...
              {SERVICES.filter(s => s.costLevel === "high" && states[s.id]).map(s => s.label).join(", ")}.
