...
import {
  ...
  ShieldCheck, Rocket, Scale, PlugZap, ChevronRight, Wallet, BotMessageSquare,
} from "lucide-react";
...
function useMarketClock() {
  ...
  useEffect(() => {
    function tick() {
      ...
      const h = gp("hour"), m = gp("minute"), s = gp("second");
      const tot = h * 3600 + m * 60 + s;
      const isWeekend = weekday === "Sat" || weekday === "Sun";
      const OPEN  = 9 * 3600 + 30 * 60;  // 9:30 AM ET
      const CLOSE = 16 * 3600;            // 4:00 PM ET
      const isOpen       = !isWeekend && tot >= OPEN && tot < CLOSE;
      const isPreMarket  = !isWeekend && tot < OPEN;
      const isAfterHours = !isWeekend && tot >= CLOSE;

      let secsToNext = 0;
      let nextLabel = "";
      if (isOpen) {
        secsToNext = CLOSE - tot; nextLabel = "closes";
      } else if (isPreMarket) {
        secsToNext = OPEN - tot;  nextLabel = "opens";
      } else {
        // After-hours or weekend — time until next 9:30 AM Mon–Fri
        const daysAhead = weekday === "Fri" ? 3 : weekday === "Sat" ? 2 : weekday === "Sun" ? 1 : 1;
        secsToNext = (86400 - tot) + OPEN + (daysAhead - 1) * 86400;
        nextLabel = "opens";
      }
      const hh = Math.floor(secsToNext / 3600);
      const mm = Math.floor((secsToNext % 3600) / 60);
      const ss = secsToNext % 60;
      const fmt = (n: number) => String(n).padStart(2, "0");
      setClock({ isOpen, isPreMarket, isAfterHours, isWeekend, nextLabel,
        countdown: `${fmt(hh)}:${fmt(mm)}:${fmt(ss)}`,
        etTime: `${fmt(h)}:${fmt(m)} ET`, weekday });
    }
    tick();
...
// ── Market Banner (countdown + prev session) ──────────────────────────────────

function MarketBanner() {
  const clk = useMarketClock();
  const [prevMoves, setPrevMoves] = useState<any[]>([]);

  useEffect(() => {
    // Fetch recent trades — best moves from yesterday / last session
    fetch("/api/trader/history?type=trades&limit=200")
      .then(r => r.json())
      .then((trades: any[]) => {
        ...
        trades.forEach(t => {
          const d = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York",
            year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(t.logged_at));
          (byDay[d] = byDay[d] || []).push(t);
        });
        const days = Object.keys(byDay).sort().reverse(); // most recent first
        // Try to get previous session (not today)
        const todayET = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York",
          year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date());
        const prevDay = days.find(d => d !== todayET) || days[0];
        if (!prevDay) return;
        const session = byDay[prevDay];
        // Sort: settled with P&L first (by P&L desc), then by notional desc
        const sorted = [...session].sort((a, b) => {
          if (a.pnl != null && b.pnl != null) return parseFloat(b.pnl) - parseFloat(a.pnl);
          if (a.pnl != null) return -1;
          if (b.pnl != null) return 1;
          return (b.notional || 0) - (a.notional || 0);
        });
        setPrevMoves(sorted.filter(t => t.side === "buy").slice(0, 5));
      }).catch(() => {});
  }, []);

  if (!clk) return null;

  const status = clk.isOpen ? "open"
    : clk.isPreMarket ? "pre"
    : clk.isAfterHours ? "after"
    : "closed";

  const statusCfg: any = {
    open:   { label: "Market Open",   dot: "bg-emerald-500 animate-pulse", bar: "border-emerald-500/20 bg-emerald-500/5", txt: "text-emerald-600 dark:text-emerald-400" },
    pre:    { label: "Pre-Market",    dot: "bg-yellow-400",                 bar: "border-yellow-500/20 bg-yellow-500/5",   txt: "text-yellow-600 dark:text-yellow-400" },
    after:  { label: "After Hours",   dot: "bg-blue-400",                   bar: "border-blue-500/20 bg-blue-500/5",       txt: "text-blue-600 dark:text-blue-400" },
    closed: { label: "Market Closed", dot: "bg-muted-foreground",           bar: "border-border bg-muted/30",              txt: "text-muted-foreground" },
  };
  const cfg = statusCfg[status];

  return (
    <div className={`rounded-xl border mb-5 px-4 py-3 ${cfg.bar}`}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        {/* Left: status + countdown */}
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <span className={`inline-block w-2 h-2 rounded-full ${cfg.dot}`}/>
            <span className={`text-sm font-semibold ${cfg.txt}`}>{cfg.label}</span>
            <span className="text-[10px] text-muted-foreground">({clk.etTime})</span>
          </div>
          <div className="flex items-center gap-1.5">
            <Clock className="h-3 w-3 text-muted-foreground"/>
            <span className="text-[10px] text-muted-foreground capitalize">{clk.nextLabel}</span>
            <span className={`text-sm font-mono font-bold tabular-nums ${cfg.txt}`}>{clk.countdown}</span>
          </div>
        </div>

        {/* Right: prev session best moves */}
        {prevMoves.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Prev session top picks:</span>
            {prevMoves.slice(0, 5).map((t, i) => (
              <div key={i} className="flex items-center gap-1 rounded border border-border bg-background/60 px-2 py-0.5">
                <span className="text-[11px] font-semibold font-mono">{t.symbol}</span>
                {t.pnl != null ? (
                  <span className={`text-[10px] font-mono ${parseFloat(t.pnl) >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}>
                    {parseFloat(t.pnl) >= 0 ? "+" : ""}{parseFloat(t.pnl).toFixed(2)}
                  </span>
                ) : (
                  <span className="text-[10px] text-muted-foreground">${(t.notional||0).toLocaleString("en-US",{maximumFractionDigits:0})}</span>
                )}
              </div>
            ))}
          </div>
        ...
        {prevMoves.length === 0 && (
          <span className="text-[10px] text-muted-foreground">No previous session data yet</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <AdminLayout>
      <div>
        ...
        <TradingPageHeader
          title="Claude Trader"
          subtitle="Autonomous AI trading across Day, Swing, Portfolio &amp; Crypto strategies via Alpaca"
          icon={<BotMessageSquare className="h-6 w-6" />}
          accentClass="text-teal-500"
          loading={!account}
          balance={account ? [
            { label: "Portfolio Equity",  value: fmtUSD(account.equity),      color: "default"  },
            { label: "Cash",              value: fmtUSD(account.cash),         color: "blue"     },
            { label: "Buying Power",      value: fmtUSD(account.buyingPower),  color: "default"  },
            { label: "Long Value",        value: fmtUSD(positions.reduce((s:number,p:any)=>s+(p.mktVal||0),0)), color: "default", sub: `${positions.length} pos` },
            { label: "Today P&L",         value: fmtUSD(Math.abs(account.pnl)), color: pnlColor(account.pnl) },
            { label: "Today %",           value: fmtPct(account.pnlPct),       color: pnlColor(account.pnlPct) },
          ] : []}
        />
