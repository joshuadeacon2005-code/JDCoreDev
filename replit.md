...
- **TradingPageHeader**: Shared React component (`client/src/components/TradingPageHeader.tsx`) used across all four trading pages (Trader, Predictions, Arbitrage, Crypto Arb). Renders a colour-coded title/icon row, a grid of balance tiles (live Kalshi + Polymarket balances, P&L), and an analytics stats bar. Accepts `balance[]`, `stats[]`, and `badges` props; loading state shows skeleton placeholders. Helper exports: `pnlColor`, `fmtUSD`, `fmtPct`.
