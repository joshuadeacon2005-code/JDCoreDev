/**
 * Claude Trader — Express router (Alpaca + market signals)
 * Mounted at /api/trader/*
 *
 * The legacy server-side cron + modelfarm Claude SDK pipeline was retired.
 * Decisions are now produced by an Anthropic-hosted scheduled routine
 * (server/trader-agent.ts, mounted at /api/trader/agent). This router only
 * exposes admin-facing data + execution helpers.
 */
...
// ── Market Signals (Yahoo screeners + StockTwits trending) ─────────────────
...
// ── Init ────────────────────────────────────────────────────────────────────
// Legacy server-side cron + modelfarm Claude SDK pipeline retired in W3 cleanup.
// /api/trader/cron/run, runServerPipeline, MODE_INTERVAL_MINUTES, isMarketHours,
// shouldRunNow, and the cron.schedule block were all removed. Decisions now come
// from the Anthropic-hosted scheduled routine via /api/trader/agent/*.

export async function initTrader() {
  await initTraderTables();
  console.log('[trader] tables ready — agent-routine path active');
}
