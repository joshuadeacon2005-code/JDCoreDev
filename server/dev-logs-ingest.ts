...
// Update a Claude-Code auto-log's description and/or minutesSpent. Used by
// migration scripts to repair entries (broken-format descriptions, inflated
// minutes from old GAP_CAP_MS=5min). Scoped to category=claude-code-session
// so this can't be used to mutate human-entered logs.
const patchSchema = z
  .object({
    description: z.string().min(1).optional(),
    minutesSpent: z.number().int().min(0).optional(),
  })
  .refine(d => d.description !== undefined || d.minutesSpent !== undefined, {
    message: "must provide description or minutesSpent",
  });
...
devLogsIngestRouter.patch("/:id", requireApiKey, async (req, res, next) => {
  try {
    ...
    const updates: { description?: string; minutesSpent?: number } = {};
    if (parsed.data.description !== undefined) updates.description = parsed.data.description;
    if (parsed.data.minutesSpent !== undefined) updates.minutesSpent = parsed.data.minutesSpent;
