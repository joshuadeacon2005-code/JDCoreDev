...
async function callClaudeTrader(prompt: string, useSearch = false): Promise<string> {
  const body: any = {
...
traderRouter.post('/cron/run', async (req, res) => {
  ...
  try {
    ...
    const pipeline = await runServerPipeline(
  ...
  cronJob = cron.schedule(schedule, async () => {
    ...
    for (const mode of ['swing']) {
      ...
      try {
        ...
        const pipeline = await runServerPipeline({ risk, mode, equity: cronEquity, buyingPower: cronBp, keys: cronKeys }, async (s, st, msg) => {
        ...
        // Reconcile P&L from Alpaca fill activities
        try {
