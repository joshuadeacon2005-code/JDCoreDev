...
export default function TraderPerformance() {
  ...
  return (
    <AdminLayout>
      <div>
        <TraderNav active="/admin/trader/performance"/>
        <div className="flex items-start justify-between mb-5">
          ...
          <Button variant="outline" size="sm" onClick={syncPnl} disabled={syncing} className="flex items-center gap-1.5 text-xs mt-1">
            <RefreshCw className={cn("h-3.5 w-3.5", syncing && "animate-spin")}/>
            {syncing ? "Syncing…" : "Sync P&L"}
          </Button>
        </div>
