...
import { alreadyContacted, markContacted, getAllContacted, deleteContacted } from './db.js';
import { getDrafts, markDraftSent, deleteDraft } from './draft-queue.js';
import { log }              from './logger.js';
import { initDbBridge, dbGetAllAudits, dbGetAllDrafts, dbMarkDraftSent, dbDeleteAudit, dbDeleteDraft, dbUpdateAuditStatus, dbUpdateDraft, dbUpsertAudit, dbGetSettings, dbSaveSettings } from './db-bridge.js';
import fs   from 'fs';
...
async function runRegenPipeline(record) {
  ...
  setRegenProgress(domain, { auditId: record.id, name: record.name, domain, stage: 'researching', percent: 15, startedAt: Date.now() });
  try {
    const lead = {
      name:      record.name,
      domain,
    ...
    try {
      ...
      const existingDraft = allDrafts.find(d => d.domain === domain && !d.sent);
      if (existingDraft) {
        await dbUpdateDraft(existingDraft.id, { domain, subject: outreach.subject, body: outreach.body, auditUrl });
        log(`[Regenerate] Updated draft #${existingDraft.id} for ${record.name}`);
      } else {
      ...
      log(`[Regenerate] Outreach step failed for ${record.name}: ${outreachErr.message} — continuing`);
    }
    ...
    setRegenProgress(domain, { stage: 'failed', percent: 0, error: err.message });
...
leadEngineRouter.post('/run', requireSecret, async (req, res) => {
  ...
  runLeadEngine()
    .catch(err => log(`[LeadEngine] Fatal error job ${jobId}: ${err.message}`))
...
leadEngineRouter.get('/progress', requireSecret, (req, res) => {
  ...
  for (let i = last100.length - 1; i >= 0; i--) {
    if (last100[i].includes('Lead Engine started')) { runStart = i; break; }
  }
  const runLines = last100.slice(runStart);

  const totalMatch = runLines.find(l => l.includes('Found') && l.includes('leads'));
  const totalLeads = totalMatch ? parseInt(totalMatch.match(/Found (\d+)/)?.[1] || '5') : 5;
  const processedCount = runLines.filter(l => l.includes('── Processing:')).length;
  ...
  if (done) {
    ...
    const lastStageLine = [...runLines].reverse().find(l => /Stage \d+:/.test(l));
    const stageNum = lastStageLine ? parseInt(lastStageLine.match(/Stage (\d+)/)?.[1] || '1') : 1;
    const stageNames = ['Research', 'Audit', 'Generate Page', 'Outreach', 'Save'];
    ...
    percent = Math.min(95, Math.round((leadProgress + stageProgress) * 90) + 10);
  } else if (totalMatch) {
    stageLabel = `Found ${totalLeads} leads, starting audits…`;
    percent = 10;
  }

  res.json({ running: engineRunning, percent, stage: stageLabel, lines: runLines.slice(-20), done });
});
...
leadEngineRouter.post('/manual-audit', requireSecret, async (req, res) => {
  ...
  if (!url && !instagram) return res.status(400).json({ error: 'url or instagram required' });
  ...
  if (await alreadyContacted(domain)) {
    return res.status(409).json({ error: `Already audited: ${instagram || domain}` });
  }
  ...
  try {
    log(`[Manual] Starting audit for ${domain}${igHandle ? ` (${igHandle})` : ''}`);

    const rawName = cleanUrl
      ? domain.replace(/\.(com|hk|sg|my|co|net|org).*$/, '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
      : igHandle.replace('@', '').replace(/[._-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    ...
    await markContacted(domain, lead.name, auditUrl, emailed ? 'email' : 'manual');
...
leadEngineRouter.post('/re-audit-draft', requireSecret, async (req, res) => {
  ...
  if (url && url.trim()) {
    ...
    return res.status(400).json({ error: 'No website domain available for this draft. Please enter a URL.' });
  }

  // Return immediately — run async (progress tracked via regenProgress map)
  res.json({ started: true, draftId, domain });

  setRegenProgress(domain, { name: draft.company, domain, stage: 'researching', percent: 15, startedAt: Date.now() });

  try {
    log(`[ReAudit] Starting re-audit for draft #${draftId} (${draft.company}) → ${domain}`);
...
leadEngineRouter.post('/rewrite-message', requireSecret, async (req, res) => {
  ...
  try {
    ...
    log(`[RewriteMsg] Rewriting draft #${id} for ${draft.company} in tone: ${tone}`);
    ...
    res.json({ subject: result.subject, body: result.body, shortMessage: result.shortMessage });
  } catch (err) {
...
leadEngineRouter.post('/regenerate-audit', requireSecret, async (req, res) => {
  ...
  res.json({ started: true, auditId, name: record.name, domain: record.domain });
...
leadEngineRouter.post('/regenerate-all-missing', requireSecret, async (req, res) => {
  const allAudits = await dbGetAllAudits();
  ...
  for (const record of missing) {
    setRegenProgress(record.domain, { auditId: record.id, name: record.name, domain: record.domain, stage: 'queued', percent: 0, startedAt: Date.now() });
  }
  res.json({ queued: missing.length, names: missing.map(a => a.name), domains: missing.map(a => a.domain) });
...
leadEngineRouter.post('/dedup-cleanup', requireSecret, async (req, res) => {
  ...
  for (const [key, group] of Object.entries(groups)) {
    ...
    for (const dupe of dupes) {
      log(`[Dedup] Removing duplicate "${dupe.domain}" (keeping "${keep.domain}")`);
...
leadEngineRouter.post('/send-draft', requireSecret, async (req, res) => {
  ...
  if (!recipient) return res.status(400).json({ error: 'No email address — provide one in the request body as "to"' });
