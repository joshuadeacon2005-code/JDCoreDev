...
Where a primitive needs server-side state, secrets, or live-API auth (Camoufox session reuse, Fincept API key not visible to the routine, anything signing or hitting an authenticated venue), the second half of the pattern kicks in: add a thin endpoint to the JDCoreDev Express API behind `x-jdcd-agent-key`, identical to how `trader-agent.ts` and `predictor-agent.ts` are wired today. The project-level skill is then a thin client over that endpoint.
...
Same pattern: project-level skill at `.claude/skills/fincept-fundamentals/SKILL.md` + Express endpoint (e.g. `GET /api/trader/fincept/:dataset/:ticker`) behind `x-jdcd-agent-key` that holds the Fincept API key in Railway env. Skill returns source-tagged blocks (provider + dataset) so attribution survives into the routine's research output. Toggle (env var or skill arg) disables per-run; default ON for Paper, opt-in for Live (ROADMAP.md Phase 5 acceptance criterion 4).
