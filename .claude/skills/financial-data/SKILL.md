---
name: financial-data
description: Fetch source-attributed financial data for a ticker or macro indicator — FMP fundamentals (income statement, balance sheet, cash flow — three statements combined into one envelope), AlphaVantage news with sentiment scores via NEWS_SENTIMENT, FMP end-of-day prices via historical-price-full, FRED macro time-series, and FRED series search — via JDCoreDev's /api/trader/data endpoint family. Every response carries provider (fmp|alphavantage|fred) + dataset attribution so the routine can cite where each number came from in research output.
---

# financial-data

A routine-callable primitive that wraps JDCoreDev's `/api/trader/data` endpoint
family. Use it when the routine needs source-tagged company fundamentals (FMP
free tier), news with per-article sentiment (AlphaVantage free tier), EOD prices
(FMP, fallback / cross-check vs Alpaca), or FRED macro time-series, with
provenance preserved into the research output. Three providers (FMP +
...
- Research output needs verifiable provenance — a citation like "FMP
  fundamentals AAPL 2026-05-08" rather than an unsourced number.
- Fundamentals comparison (income statement / balance sheet / cash flow,
  combined into a single envelope from FMP's three statement endpoints).
...
- High-volume fundamentals scans — FMP free tier is **250 calls/day**, and a
  single fundamentals request burns 3 (one each for income / balance /
  cashflow) → ~83 fundamentals requests/day budget.
...
```
{
  ...
  "fetched_at":       "2026-05-08T13:24:01.234Z",
  "source_url":       "https://financialmodelingprep.com/api/v3/... | https://www.alphavantage.co/... | https://api.stlouisfed.org/...",
...
### Worked example — FMP fundamentals (AAPL)

`curl -H "x-jdcd-agent-key: $JDCD_AGENT_KEY" .../api/trader/data/fundamentals/AAPL`
→ envelope with `provider: "fmp"`, `dataset: "fundamentals"`,
`ticker_or_series: "AAPL"`,
`source_url: "https://financialmodelingprep.com/api/v3/income-statement/AAPL?limit=5"`,
and `data: { income_statement: [...5 most recent annual statements...], balance_sheet: [...], cash_flow: [...] }`
— three FMP statement endpoints (`income-statement`, `balance-sheet-statement`,
`cash-flow-statement`) called in parallel and combined into one envelope. **Each
fundamentals call burns 3 of the 250 daily quota** → ~83 fundamentals
requests/day budget.
...
|---|---|---|---|---|---|
| `fundamentals` | `/:dataset/:ticker`   | fmp          | stock symbol     | `{ income_statement: [...], balance_sheet: [...], cash_flow: [...] }` (5 most recent annual statements per array, combined from FMP's three statement endpoints) | Fundamentals comparison (1 request = 3 of 250 daily quota) |
...
| `prices_eod`   | `/:dataset/:ticker`   | fmp          | stock symbol     | `{ ohlcv: [{ date, open, high, low, close, adjClose, volume }] }` | EOD fallback / cross-check |
...
| 429 | Upstream provider rate-limited (AlphaVantage's 5/min free-tier cap or FMP's 250/day daily cap) | Back off ≥ `retry_after` seconds (or ≥ 12 s if absent for AlphaVantage), retry once. FMP daily-cap exhaustion is operator-territory — surface and skip. |
| 500 | Network error / 20s timeout / upstream lib error | Retry once with a 30s pause. |
| 502 | Upstream non-2xx — ticker / series not found, plan-tier mismatch, FMP unreachable | Don't retry the same input more than 3 times. |
| 503 | `FMP_API_KEY` (for fundamentals / prices_eod) / `ALPHA_VANTAGE_API_KEY` (for news) / `FRED_API_KEY` (for macro) unset, OR `EXTERNAL_DATA_ENABLED=false` | Surface the `hint` field; don't retry — operator-action territory. |

503 responses include a `hint` telling a future Claude session what to
provision. Forward it; don't paper over it.
...
4. **Don't double-fetch.** FMP `prices_eod` is a fallback / cross-check, not
...
6. **FMP prices are EOD only** (daily bars via `historical-price-full`). For intra-day, use Alpaca.
7. **Respect FMP daily-cap.** Free tier is **250 calls/day**, and a single
   `fundamentals` request burns 3 (income + balance + cashflow in parallel).
   Budget ≈ 83 fundamentals requests/day. `prices_eod` is 1 call.
8. **Respect AlphaVantage rate limits.** Free tier is 500 calls/day, 5/min. A
...
9. **Do not call from outside the trading routines.** Endpoint is gated by
...
3. **Per-provider key check.** `FMP_API_KEY` (fundamentals / prices_eod),
   `ALPHA_VANTAGE_API_KEY` (news), or `FRED_API_KEY` (macro) unset → 503 with
   setup `hint`. Per-request opt-out is evaluated BEFORE this so shape-probing
   works without keys.

`/api/trader/data/ping` deliberately bypasses the gate — operators must be
able to read `enabled` + per-provider `key_configured` even when disabled.
Shape:
...
- **TRADE-FIN-01**: skill installed at `.claude/skills/financial-data/SKILL.md`
  per D-01 / D-02. One skill covers FMP, AlphaVantage, and FRED — single
  install pattern. See `docs/trading-routine-architecture.md` "Install Pattern
  Decision".
- **TRADE-FIN-02**: every accessor documented in `## Datasets` (FMP
...
- **TRADE-FIN-05**: deferred user-action — runtime test on a known ticker
  needs `FMP_API_KEY` + `ALPHA_VANTAGE_API_KEY` + `FRED_API_KEY` provisioned
  on Railway. Until then, the 503 setup-hint path is the verification surface
